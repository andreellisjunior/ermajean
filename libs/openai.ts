...
export const aiPrompt = async (
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      ...
      {
        role: 'user',
        content: `Generate a detailed and cost effective recipe tailored to my preferences. I am craving ${taste}, ${ingredients
            ? `I have the following available ingredients: ${ingredients}`
            : `I do not have specific ingredients so I am open to suggestions`
          }. I am serving ${serving} people. and I only have ${totalTime} to make this meal. This meal is intended for ${course}. ${restrictions
            ? `Please note the dietary restrictions: ${restrictions}`
            : 'I have no dietary restrictions'
          }.${isKidFriendly
            ? ' This recipe must be kid-friendly: use simple flavors, avoid excessive spice, and make it appealing to children.'
            : ''
