...
OpenAI model review corrected on 24 September 2026: both workloads now default to `gpt-6-luna` through `libs/ai/models.ts`, with low reasoning and strict structured outputs. Recipe generation retains a 6,000-token completion budget and 45s timeout; nutrition now uses a reasoning-compatible 2,000-token completion budget and retains its 30s timeout. Both disable automatic retries. Existing deployment environment overrides must be updated explicitly.

Current model comparison (standard text price per million input / output tokens): [GPT-6 Luna](https://developers.openai.com/api/docs/models/gpt-6-luna) $0.10 / $0.50, selected for focused repeatable tasks; [GPT-6 Sol](https://developers.openai.com/api/docs/models/gpt-6-sol) $2 / $10, a quality challenger if staging shows Luna is insufficient; [GPT-6 Astra](https://developers.openai.com/api/docs/models/gpt-6-astra) $10 / $50, aimed at the hardest work and not justified here without evaluation. These are published token prices, not measured cost per recipe. [Current model and migration guide](https://developers.openai.com/api/docs/guides/latest-model).

This replaces an inadequately researched recommendation of GPT-5 mini / GPT-4o mini. The new selection is documentation-based, not a live benchmark. No paid provider calls were made; model access, latency, food/allergen quality and actual usage costs remain staging gates. Nutrition remains an estimate, not verified food-database analysis. Local verification after this correction: 19 backend tests passed, including PostgreSQL integration and mocked SDK request/response tests; TypeScript and production build passed; lint passed with 29 existing warnings. The original verification log remains historical evidence from the earlier pass.
